...
--
-- NULL handling matters here: percent_rank sorts NULLs last, so a filer
-- with no capex figure would rank as the best FCF margin in its group.
-- Each rank therefore partitions on whether the metric is NULL and returns
-- NULL for the NULL side, so a missing input yields a missing rank.
WITH latest AS (
    SELECT *
    FROM core.annual
    WHERE revenue > 0 AND total_assets > 0 AND net_income IS NOT NULL
...
SELECT
    ...
    CASE WHEN fcf_margin IS NULL THEN NULL
         ELSE round(percent_rank() OVER (PARTITION BY sic2, fiscal_year, fcf_margin IS NULL
                                         ORDER BY fcf_margin), 3) END                      AS fcf_margin_pctile,
    count(fcf_margin) OVER (PARTITION BY sic2, fiscal_year)                                AS fcf_peers
FROM ratios
WHERE peers >= 20
